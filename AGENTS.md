...
6. **Supported API hosts today:** **`host: fly`**, **`host: railway`**, **`host: digitalocean`** (`do`), **`host: render`**, **`host: cloud_run`** (`gcp`), **`host: aws`** (App Runner), **`host: aws_ecs`** (Fargate+ALB), **`host: azure`** (Container Apps). Examples: https://github.com/127thousand/podfly_examples
7. **Doctor does not require Fly** unless `host: fly` (or default). Railway → `railway` CLI; DigitalOcean → `doctl` + Docker + DOCR; AWS → `aws` + Docker + ECR; Azure → `az` + Docker + ACR. Pages still needs `wrangler` when `mode: split` and web enabled on Cloudflare.
7b. **App Runner has no WebSockets** (managed Envoy 403 on Upgrade). AWS streams → **`host: aws_ecs`** (Fargate+ALB). Azure Container Apps supports WS. Details: `doc/aws.md`, `doc/azure.md`.
