...
7. **Doctor does not require Fly** unless `host: fly` (or default). Railway → `railway` CLI; DigitalOcean → `doctl` + Docker + DOCR; AWS → `aws` + Docker + ECR; Azure → `az` + Docker + ACR; Hetzner → `hcloud` + Docker + SSH. Pages still needs `wrangler` when `mode: split` and web enabled on Cloudflare.
